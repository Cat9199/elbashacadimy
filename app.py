...
CLIENT_ID = "275922294832-77l8gjef21pjh3th3bnhi7d2h0h0oe9p.apps.googleusercontent.com"
CLIENT_SECRET = "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"
REDIRECT_URI = "http://elbashaacademy.e3lanotopia.software/callback"
...
def playvideo(id):
    ...
    if username:
        ...
        if lesson.Ltype == 'section':
            return redirect(f'/playvideo/{int(lesson.id)+1}')
        else:
            pass
        ...
        if enroll:
            ...
            db.session.commit()
            if session['is_mobile']:
                return render_template('app/dashboard/playvideo.html',user=user,lesson=lesson,course=all_lessons)
            else :
                return render_template('web/dashboard/playvideo.html',user=user,lesson=lesson,course=all_lessons)
        else:
